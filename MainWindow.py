...
# Import necessari alla trasformazione da risultato di yolo(tensor array) in immagine da passare alla PixMap
...
class Ui_MainWindow(object):
    ...
    def setupUi(self, MainWindow):
        ...
        # Creo il painter che userò poi come canvas per disegnare i bounding box che mi restituirà
    ...
    def infer(self):
        if self.file_path == '' or self.file_path is None:
            d = QDialog()
            d.setFixedSize(225, 150)
            lbl = QLabel("Selezionare un frame o un video\nprima di effettuare l'inferenza.", d)
            lbl.move(35, 50)
            b1 = QPushButton("OK", d)
            b1.clicked.connect(lambda: d.close())
            b1.move(75, 100)
            d.setWindowTitle("Attenzione")
            d.exec_()
            return
    ...
    def yolov5_inference(self, path, model_size="small", conf_threshold=.75):
        ...
        # Check if the weight file is there
        if not os.path.exists(weight_path):
            ...
            try:
                ...
                # TODO dialog che avverte l'utente che c'è stato un problema nello scaricare i pesi
                pass

        # Setup the model with custom parameters
        ...
        try:
            # Salvo il risultato in modo da poterlo renderizzare al posto dell'immagine senza BB
            results.save(save_dir=save_path, exist_ok=True)
        except:
            # TODO dialog che avverte l'utente che c'è stato un problema nel salvataggio/rendere dell'immagine
            pass

        self.PhotoWidget.setPixmap(
            QtGui.QPixmap(f"{save_path}/{self.file_path.split('/')[-1]}"))
    ...
    def yolov6_inference(self, model_size="small", conf_threshold=.75):
        pass

    def yolov7_inference(self, model_size="small", conf_threshold=.75):
        pass
